        ...
        )
{
    ...
    // UVLM::Types::Vector3 r1 = v1 - rp;
    // UVLM::Types::Vector3 r2 = v2 - rp;
    ...
    // UVLM::Types::Vector3 K;
    // K = (gamma/(UVLM::Constants::PI4*r1_cross_r2_mod_sq)*r1_cross_r2).dot(r0)*
    //     (r1/r1_mod - r2/r2_mod);
    //     std::cout << "rp = " << rp.transpose() << std::endl;
    //     std::cout << "v1 = " << v1.transpose() << std::endl;
    //     std::cout << "v2 = " << v2.transpose() << std::endl;
...
)
{
    ...
    for (unsigned int i_segment=0; i_segment<n_segment; ++i_segment)
    {
        ...
        // std::cout << "start = " << UVLM::Mapping::vortex_indices(start,0) << "  "
        //           << UVLM::Mapping::vortex_indices(start,1) << std::endl;
        // std::cout << "end = " << UVLM::Mapping::vortex_indices(end ,0) << "  "
        //           << UVLM::Mapping::vortex_indices(end,1) << std::endl;
        // uint a;
        // std::cin >> a;
    ...
    for (unsigned int i_col=0; i_col<rows_collocation; ++i_col)
    {
        for (unsigned int j_col=0; j_col<cols_collocation; ++j_col)
        {
            ...
            target_triad << target_surface[0](i_col, j_col),
                            ...
                            target_surface[2](i_col, j_col);
            UVLM::Types::initialise_VecMat(temp_uout, 0.0);
            UVLM::BiotSavart::surface_with_horseshoe(zeta,
                                                     zeta_star,
                                                     gamma,
                                                     gamma_star,
                                                     target_triad,
